...
- [x] T039 Confirm with a screen reader that replacing the advice is announced politely and that focus is not stolen from the dice button (FR-015, research D10). **OMITTED — descoped by the author on 2026-09-21. The listening pass with a real screen reader was never performed. What IS verified: the accessibility tree reports `status atomic live="polite"` on the advice region, focus is never moved programmatically, and the Frontend Mentor report scored Accessibility 9.4/10. The box is checked to close the task, not to claim the check was run.**
...
- [x] T041 Work through every check in [quickstart.md](./quickstart.md) and confirm all 20 pass. **19 of 20 verified in the browser. Check 17, the screen-reader announcement, was omitted along with T039.**
